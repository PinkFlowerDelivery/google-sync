...
pub async fn load_config() -> Result<Config, std::io::Error> {
    ...
    if let Ok(file) = fs::try_exists(&path_file).await {
        if !file {
            ...
            fs::write(&path_file, default_config_string).await.unwrap();
        }
    }

    let context = fs::read_to_string(&path_file).await.unwrap();
    let loaded_config: Config = toml::from_str(&context).unwrap();
